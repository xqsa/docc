...
RUNS_ROOT = ARTIFACTS_ROOT / "hcc_fidelity_patch_v1_runs"
RUN_DETAILS_PATH = ARTIFACTS_ROOT / "hcc_fidelity_patch_v1_run_details.csv"
VISIBILITY_PATH = ARTIFACTS_ROOT / "hcc_fidelity_patch_v1_visibility.csv"
SUMMARY_PATH = ARTIFACTS_ROOT / "hcc_fidelity_patch_v1_summary.csv"
REPORT_PATH = ARTIFACTS_ROOT / "hcc_fidelity_patch_v1_report.md"
...
METHOD_CONFIGS = [
    ...
    ("topology-order-original", CONFIG_ROOT / "topology-original.json"),
    ("topology-order-eq8-correct", CONFIG_ROOT / "topology-eq8-correct.json"),
    ("hypergraph-selective-conflict", CONFIG_ROOT / "selective-hypergraph-conflict.json"),
...
VISIBILITY_FIELDNAMES = [
    ...
    "original_adjacent_overlap_count",
    "original_adjacent_overlap_ratio",
    "ordered_adjacent_overlap_count",
    "ordered_adjacent_overlap_ratio",
...
def parse_args():
    parser = argparse.ArgumentParser(description="Generate HCC-fidelity patch V1 artifacts.")
...
def build_visibility_rows(method_configs, problems):
    ...
    for method_name, config_path in method_configs:
        ...
        for problem_code in problems:
            ...
            natural_groups = hcc_es.build_aob_natural_groups(problem_code)
            if config.group_order_mode == "aob_topology":
                _, _, order_audit = hcc_es.apply_group_order_mode(
                    inputs["grouping_result"],
                    problem_code=problem_code,
                    group_order_mode=config.group_order_mode,
                )
            else:
                order_audit = hcc_es.build_group_order_visibility_audit(
                    inputs["grouping_result"],
                    inputs["grouping_result"],
                    natural_groups,
                )
                order_audit["group_order_mode"] = config.group_order_mode
                order_audit["group_order_changed"] = False
            rows.append(
                {
...
def run_one_case(method_name, config, problem_code, seed, tfes):
    ...
    try:
        ...
        shared_summary = dict(metadata.get("shared_variable_coordination_summary", {}))
        return {
            "problem": str(problem_code).upper(),
            "seed": int(seed),
            "tfes": int(tfes),
            "method": method_name,
            "group_order_mode": str(metadata.get("group_order_mode", config.group_order_mode)),
            ...
            "shared_variable_fusion_count": int(shared_summary.get("total_updated_count", 0) or 0),
        ...
        return {
            ...
            "tfes": int(tfes),
            "method": method_name,
            "group_order_mode": str(config.group_order_mode),
            ...
            "shared_variable_fusion_count": 0,
            "status": f"error: {exc}",
        }


def summarize_rows(run_rows, visibility_rows):
    visibility_by_key = {
        (row["problem"], row["method"]): to_float(row.get("ordered_adjacent_overlap_ratio"))
    ...
    for (problem, tfes, method), rows in grouped.items():
        ...
        if method == "no-coordination":
            no_coord_best_by_problem_tfes[(problem, tfes)] = best_mean
    ...
    for key in sorted(grouped):
        ...
        no_coord_best = no_coord_best_by_problem_tfes.get((problem, tfes), float("nan"))
        summary.append(
            {
                "problem": problem,
                "tfes": int(tfes),
                ...
                "gap_vs_old_best": float((best_mean - old_best) / max(abs(old_best), 1e-300)) if np.isfinite(best_mean) and np.isfinite(old_best) else float("nan"),
                "gap_vs_no_coordination_best": float((best_mean - no_coord_best) / max(abs(no_coord_best), 1e-300)) if np.isfinite(best_mean) and np.isfinite(no_coord_best) else float("nan"),
                "visibility_mean": visibility_by_key.get((problem, method), float("nan")),
...
def report_table(summary_rows, tfes):
    ...
    lines = [
        f"## TFEs = {int(tfes)}",
        "",
        "| problem | method | visibility | best_mean | final_mean | gap_vs_old | gap_vs_no_coord |",
        "| --- | --- | ---: | ---: | ---: | ---: | ---: |",
    ]
    for row in rows:
        lines.append(
            "| {problem} | {method} | {visibility:.3f} | {best} | {final} | {gap_old} | {gap_no} |".format(
                problem=row["problem"],
                method=row["method"],
                visibility=to_float(row.get("visibility_mean")),
                best=format_metric(row.get("best_mean")),
                final=format_metric(row.get("final_mean")),
                gap_old=format_percent(row.get("gap_vs_old_best")),
                gap_no=format_percent(row.get("gap_vs_no_coordination_best")),
            )
        )
    return "\n".join(lines)
...
def write_report(summary_rows, visibility_rows, args):
    visibility_gain_rows = [
        row
        for row in visibility_rows
        if row["method"] == "topology-order-original"
    ]
    report = [
        "# HCC-Fidelity Patch V1",
        "",
        "- Executor: Codex",
        "- Date: 2026-05-20",
        ...
        "## Visibility Check",
        "",
        "| problem | original visibility | topology visibility | gain |",
        "| --- | ---: | ---: | ---: |",
    ]
    for row in visibility_gain_rows:
        report.append(
            "| {problem} | {old:.3f} | {new:.3f} | {gain:+.3f} |".format(
                problem=row["problem"],
                old=to_float(row.get("original_adjacent_overlap_ratio")),
                new=to_float(row.get("ordered_adjacent_overlap_ratio")),
                gain=to_float(row.get("visibility_gain")),
            )
        )
...
def main():
    ...
    summary_rows = summarize_rows(run_rows, visibility_rows)
