...
class Optimizer(object):
    ...
    def _evaluate_fitness(self, x_batch, args=None):
        ...
        参数：
        - x_batch: 输入样本矩阵，每一行是一个输入样本。
        - args: 额外参数，可选。

        返回：
        - y_batch: 每个输入样本对应的目标函数值列表。
        """
        self.start_function_evaluations = time.time()
        x_array = np.asarray(x_batch)

        if args is None:
            y_batch = self.fitness_function(x_array)
        else:
            y_batch = self.fitness_function(x_array, args=args)

        self.time_function_evaluations += time.time() - self.start_function_evaluations
        y_array = np.asarray(y_batch, dtype=float).reshape(-1)

        if x_array.ndim == 1:
            if y_array.size != 1:
                raise ValueError('fitness_function must return a scalar for a single candidate.')
            y_value = float(y_array[0])
            self.n_function_evaluations += 1
            if y_value < self.best_so_far_y:
                self.best_so_far_x, self.best_so_far_y = np.copy(x_array), y_value
            if (self._base_early_stopping - y_value) <= self.early_stopping_threshold:
                self._counter_early_stopping += 1
            else:
                self._counter_early_stopping, self._base_early_stopping = 0, y_value
            return y_value

        if x_array.ndim != 2:
            raise ValueError('x_batch must be a 1D candidate or 2D candidate batch.')
        if y_array.size != x_array.shape[0]:
            raise ValueError('fitness_function must return one value per candidate.')

        self.n_function_evaluations += x_array.shape[0]

        min_index = int(np.argmin(y_array))
        if y_array[min_index] < self.best_so_far_y:
            self.best_so_far_x, self.best_so_far_y = np.copy(x_array[min_index]), float(y_array[min_index])

        for y_value in y_array:
            if (self._base_early_stopping - y_value) <= self.early_stopping_threshold:
                self._counter_early_stopping += 1
            else:
                self._counter_early_stopping, self._base_early_stopping = 0, float(y_value)
