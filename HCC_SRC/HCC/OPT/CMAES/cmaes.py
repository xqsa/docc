...
from HCC.OPT.CMAES.es import ES   # abstract class of all Evolution Strategies (ES) classes
...
class CMAES(ES):
    ...
    def iterate(self, x=None, mean=None, e_ve=None, e_va=None, y=None, d=None, args=None):
        ...
        if remaining <= 0:
            return x, y, d
        evaluate_count = min(self.n_individuals, remaining)
        y[:evaluate_count] = self._evaluate_fitness(x[:evaluate_count], args)
